class Resource < ActiveRecord::Base
  ...
  def self.search(search)
    ...
    find_by_sql ['SELECT DISTINCT ON resources.* FROM resources INNER JOIN resources_terms ON resources_terms.resource_id = resources.id INNER JOIN terms ON terms.id = resources_terms.term_id WHERE ((description ILIKE ?) OR (title ILIKE ?) OR (alt_titles ILIKE ?))', search_condition, search_condition, search_condition]
  end
