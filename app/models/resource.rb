class Resource < ActiveRecord::Base
  ...
  def self.search(search)
    ...
    find_by_sql ['SELECT resources.* FROM resources WHERE ((lower(description) ILIKE lower(?)) OR (lower(title) ILIKE lower(?)) OR (lower(alt_titles) ILIKE lower(?)) )', search_condition, search_condition, search_condition]
  end
