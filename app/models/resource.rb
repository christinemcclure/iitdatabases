class Resource < ActiveRecord::Base
  ...
  def self.search(search)
    ...
    find_by_sql ['SELECT resources.* FROM resources WHERE ((lower(description) LIKE lower(?)) OR (lower(title) LIKE lower(?)) OR (lower(alt_titles) LIKE lower(?)) )', search_condition, search_condition, search_condition]
  end
